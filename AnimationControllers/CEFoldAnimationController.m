...
- (void)doAnim:(UIView*)fromV toV:(UIView*)toV duration:(NSTimeInterval)duration target:(id)target onComplete:(SEL)onComplete{
  toV.frame = fromV.frame;
  CGRect orgR = toV.frame;
  toV.frame = CGRectOffset(toV.frame, toV.frame.size.width, 0);
  // Add a perspective transform
  CATransform3D transform = CATransform3DIdentity;
  transform.m34 = -0.005;
  fromV.superview.layer.sublayerTransform = transform;
  
  CGSize size = toV.frame.size;
  ...
  for (int i=0 ;i<self.folds; i++){
    ...
    UIView *leftFromViewFold = [self createSnapshotFromView:fromV afterUpdates:NO location:offset left:YES];
    ...
    UIView *rightFromViewFold = [self createSnapshotFromView:fromV afterUpdates:NO location:offset + foldWidth left:NO];
    ...
    UIView *leftToViewFold = [self createSnapshotFromView:toV afterUpdates:YES location:offset left:YES];
    ...
    UIView *rightToViewFold = [self createSnapshotFromView:toV afterUpdates:YES location:offset + foldWidth left:NO];
  ...
  fromV.frame = CGRectOffset(fromV.frame, fromV.frame.size.width, 0);
